...
class IndexController extends Controller
{
    ...
    public function BuyPropertySearch(Request $request){
        ...
        ->whereHas('pstate',function($q) use($sstate){ $q->where('state_name','like', '%' .$sstate. '%'); })
        ->whereHas('type',function($q) use($stype){ $q->where('type_name','like', '%' .$stype. '%'); })->get();
    ...
    public function AllPropertySearch(Request $request){
        ...
        $property=Property::where('status','1')->where('bedrooms',$bedrooms)->where('bathrooms','like', '%' .$bathrooms. '%')
